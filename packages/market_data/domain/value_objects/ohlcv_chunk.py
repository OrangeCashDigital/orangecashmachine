# -*- coding: utf-8 -*-
"""
market_data/domain/value_objects/ohlcv_chunk.py
=================================================

OHLCVChunk — Value Object de un fragmento (chunk) del stream OHLCV.

Diferencia con OHLCVBatch (deprecado)
--------------------------------------
El nombre "OHLCVBatch" generaba ambigüedad con batch processing
(Lambda architecture), pero el sistema implementa Kappa Architecture:
todo es stream, no hay procesamiento batch offline.

"Chunk" = fragmento acotado y transportable del stream continuo.
Cada chunk puede provenir de REST (backfill), WebSocket (live) o replay.
...
class OHLCVSource:
    ...
    SSOT — cualquier código que necesite un source label importa desde aquí.
    No usar strings literales "rest", "live", etc. en el codebase.

    rest     : ingesta vía REST (CCXT) — backfill o incremental polling
    live     : ingesta vía WebSocket en tiempo real (cryptofeed)
    ...
    def is_valid(cls, source: str) -> bool:
        """True si source es un valor canónico conocido."""
...
class OHLCVChunk:
    """
    Fragmento inmutable de velas OHLCV con metadatos de contexto.

    Es la unidad semántica de transferencia entre adapters inbound
    y consumers del pipeline — reemplaza Tuple[CandleTuple, ...].

    Kappa Architecture: representa un fragmento acotado del stream,
    no un batch offline. Compatible con backfill, live y replay.
    ...
    @property
    def count(self) -> int:
        """Número de velas en el chunk."""
        return len(self.candles)

    @property
    def is_empty(self) -> bool:
        """True si el chunk no tiene velas."""
    ...
    def start_ms(self) -> Optional[int]:
        """timestamp_ms de la primera vela. None si el chunk está vacío."""
        if self.is_empty:
            return None
    ...
    def end_ms(self) -> Optional[int]:
        """timestamp_ms de la última vela. None si el chunk está vacío."""
        if self.is_empty:
            return None
    ...
    def from_tuples(
        ...
        """
        Construye un OHLCVChunk desde tuplas crudas CCXT.
...
__all__ = ["OHLCVChunk", "OHLCVSource"]
