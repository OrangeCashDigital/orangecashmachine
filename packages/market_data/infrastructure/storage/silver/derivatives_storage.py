...
class DerivativesStorage:
    ...
    def __init__(
        ...
        if dataset not in SUPPORTED_DATASETS:
            raise ValueError(
                f"DerivativesStorage: dataset no soportado: {dataset!r}. Soportados: {sorted(SUPPORTED_DATASETS)}"
            )
        if not exchange:
    ...
    def last_timestamp_ms(self, symbol: str) -> int | None:
        ...
        try:
            ...
            max_ts = max(v.timestamp() * 1_000_000 if hasattr(v, "timestamp") else v for v in ts_col if v is not None)
