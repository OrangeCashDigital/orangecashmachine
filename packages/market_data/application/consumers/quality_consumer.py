...
class QualityPipelineConsumer(BaseConsumer):
    ...
    def _to_dataframe(event: OHLCVBatchReceived) -> pl.DataFrame:
        ...
        if event.batch.is_empty:
            return pl.DataFrame(schema=dict.fromkeys(_CANDLE_COLS, pl.Float64))
        _schema = dict(
            zip(_CANDLE_COLS, [pl.Int64, pl.Float64, pl.Float64, pl.Float64, pl.Float64, pl.Float64], strict=True)
        )
        return pl.DataFrame(
