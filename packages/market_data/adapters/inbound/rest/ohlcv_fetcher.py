...
if TYPE_CHECKING:
    # LatenessCalibrationStore: exclusivo para anotaciones de tipo.
    # El import de runtime ocurre en _LazyCalibrationStore._get() —
...
def overlap_for_timeframe(timeframe: str, exchange: str | None = None) -> int:
    ...
    if exchange is not None:
        ...
        if lateness_ms is None:
            lateness_ms = _ALLOWED_LATENESS_MS_BY_EXCHANGE.get(exchange, _ALLOWED_LATENESS_MS_DEFAULT)
...
class HistoricalFetcherAsync:
    ...
    def __init__(
        ...
        if transformer is None:
            raise ValueError(
                "OHLCVFetcher requiere transformer inyectado. Usa pipeline_factory.py como Composition Root."
            )
    ...
    async def _download_chunked(
        ...
        if since_ts is None:
            ...
            raise MissingStartDateError(f"No se pudo resolver since_ts para {symbol}/{timeframe}")
        ...
        for chunk_idx in range(MAX_CHUNKS_PER_RUN):
            ...
            try:
                ...
                self._log.bind(symbol=symbol, timeframe=timeframe, chunk=chunk_idx).warning(
                    "Circuit open — aborting chunked download"
                )
    ...
    async def _resolve_start_timestamp(
        ...
        if self._backfill_mode:
            ...
            if not candidate:
                raise MissingStartDateError(f"backfill_mode=True requiere config_start_date | {symbol}/{timeframe}")
        ...
        for candidate in [start_date, self._config_start_date]:
            if candidate and candidate != "auto":
                self._log.bind(symbol=symbol, timeframe=timeframe, desde=candidate).info(
                    "Primer inicio — fecha configurada"
                )
                return self._exchange.parse8601(candidate)
    ...
    async def _fetch_chunk_with_retry(
        ...
        try:
            ...
            self._log.bind(
                ...
                "Adapter retries exhausted — failing chunk | error_type={} root_cause={} root_err={}",
                exc.error_type,
                type(root_cause).__name__,
                str(root_cause),
            )
            raise ChunkFetchError(
                f"{symbol}/{timeframe} retry exhausted ({exc.error_type}): {type(root_cause).__name__}: {root_cause}"
            ) from exc
            ...
            self._log.bind(
                ...
                "Unexpected error from adapter — failing chunk | error_type={} err={}",
                type(exc).__name__,
                err_str,
            )
            raise ChunkFetchError(f"{symbol}/{timeframe} unexpected error: {type(exc).__name__}: {exc}") from exc
    ...
    def _validate_market(self, symbol: str, market_type: Optional[str] = None) -> None:
        ...
        if market_type == "swap" and not market.get("swap", False):
            raise InvalidMarketTypeError(
                f"Symbol '{symbol}' is not swap/futures in {exchange_id} (type={market.get('type', '?')}) — fix config"
            )
        if market_type == "spot" and not market.get("spot", False):
            raise InvalidMarketTypeError(
                f"Symbol '{symbol}' is not spot in {exchange_id} (type={market.get('type', '?')}) — fix config"
            )
