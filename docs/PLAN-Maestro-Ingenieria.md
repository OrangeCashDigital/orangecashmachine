...
| 2026-08-23 | (B-51) | **B-51 CERRADO (Option 3)**: policies/registry.yaml = SSOT canónico para reglas R1-R16; tracking.yaml rules marcadas DEPRECATED (compatibility layer). IDs R1-R16 alineados. audit_validator M21-M25 PASS. |
...
| 2026-08-23 | (B-53) | **B-53 CERRADO**: Semgrep non-blocking adoptado (ADR-0034) — policies/semgrep/architecture.yaml (8 reglas YAML: domain-no-import-*, domain-no-os-environ, application-no-subprocess, domain-no-open, domain-no-ccxt, pandas-no-append). CI job non-blocking (continue-on-error: true). 76 findings baseline (mayoría false positives en list.append). |
