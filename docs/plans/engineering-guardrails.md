...
| 6 | Separación explícita de publishers permitidos por entorno en config | ✅ Cerrado | docstring `EnvironmentConfig.name` corregido (commit `1253c8f`); regla L5 `PRODUCTION_REQUIRES_KAFKA_PUBLISHER` en `ocm/config/layers/rules.py` (commit `60b9dcd`) adelanta a config-load el fail-fast que antes ocurría solo en runtime (`_build_ohlcv`); demostrada E2E por el pipeline real L1→L5 (`TestKafkaRuleEndToEndPipeline`, test_business_rules.py) y deploy alineado (docker-compose `KAFKA_ENABLED=${KAFKA_ENABLED:-true}`, `.env.example`/README) |
