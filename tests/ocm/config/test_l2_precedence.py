...
def test_ocm_override_wins_over_l1_value(base_cfg):
    ...
    merged, count = apply_env_overrides(base_cfg, env={"OCM_INTEGRATIONS__REDIS__HOST": "l2-host"})
...
def test_unoverridden_l1_values_preserved(base_cfg):
    ...
    merged, _ = apply_env_overrides(base_cfg, env={"OCM_INTEGRATIONS__REDIS__HOST": "new"})
...
def test_all_malformed_returns_zero_count(base_cfg):
    ...
    _, count = apply_env_overrides(base_cfg, env={"OCM_A": "x", "OCM_B": "y"})
    assert count == 0
...
def test_three_level_path_parsed_correctly(base_cfg):
    ...
    merged, count = apply_env_overrides(base_cfg, env={"OCM_INTEGRATIONS__REDIS__ENABLED": "false"})
...
def test_two_level_path_parsed_correctly(base_cfg):
    ...
    merged, _ = apply_env_overrides(base_cfg, env={"OCM_SAFETY__DRY_RUN": "true"})
    assert merged.safety.dry_run is True
...
def test_bool_recognized_values_coerced(base_cfg, raw, expected):
    ...
    merged, _ = apply_env_overrides(base_cfg, env={"OCM_SAFETY__DRY_RUN": raw})
...
def test_input_dictconfig_not_mutated(base_cfg):
    ...
    apply_env_overrides(base_cfg, env={"OCM_INTEGRATIONS__REDIS__HOST": "mutated?"})
