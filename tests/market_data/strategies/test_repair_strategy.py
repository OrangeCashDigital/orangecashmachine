...
def _raw_candles(
    ...
    return [[start_ms + i * tf_ms, 100.0, 110.0, 90.0, 105.0, 1000.0] for i in range(n)]
...
class TestExecutePairHealing:
    ...
    async def test_healed_count_full_fill(self):
        ...
        with patch("market_data.domain.value_objects.exchange_quirks.get_quirks") as mock_quirks:
    ...
    async def test_gap_too_large_skipped(self):
        ...
        from market_data.application.strategies.repair import _MAX_HEALABLE_GAP_CANDLES
...
class TestReadSilver:
    ...
    def test_columns_only_ignores_missing_columns(self):
        ...
        result = strategy._read_silver(ctx, _SYMBOL, _TF, columns_only=["timestamp", "nonexistent"])
        assert result is not None
...
class TestHealGap:
    ...
    async def test_no_data_available_marks_irrecoverable(self):
        ...
        assert any(call.get("irreversible") is True for call in registry.healed_calls)
    ...
    async def test_chunk_fetch_error_returns_false(self):
        ...
        bad_fetcher = MagicMock()
        bad_fetcher.fetch_chunk = AsyncMock(side_effect=ChunkFetchError("rate limit"))
        ctx = PipelineContext(
            fetcher=bad_fetcher,
            storage=_StorageStub(),
            cursor=MagicMock(),
        ...
        assert not any(call.get("irreversible") is True for call in registry.healed_calls)
