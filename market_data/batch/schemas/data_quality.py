...
_FLATLINE_THRESHOLD_BY_TF = {"1m":0.0001,"5m":0.0001,"15m":0.0002,"1h":0.0005,"4h":0.001,"1d":0.002}
...
class DataQualityReport:
    ...
    @property
    def criticals(self): return [i for i in self.issues if i.severity == "critical"]
    @property
    def has_critical_issues(self): return bool(self.criticals)
    @property
    def is_clean(self): return not self.issues
    @property
    def affected_rows_total(self): return sum(i.affected_rows for i in self.issues)
    def issues_by_check(self, name): return [i for i in self.issues if i.check == name]
    def summary(self):
        parts = [f"DataQualityReport | {self.symbol}/{self.timeframe} exchange={self.exchange} rows={self.rows} warnings={len(self.warnings)} criticals={len(self.criticals)} git={self.git_hash}"]
        for i in self.issues: parts.append(f"  [{i.severity.upper()}] {i.check}: {i.description} (rows={i.affected_rows})")
        return chr(10).join(parts)
    def to_dict(self):
        return {"symbol":self.symbol,"timeframe":self.timeframe,"exchange":self.exchange,"rows":self.rows,"checked_at":self.checked_at,"git_hash":self.git_hash,"warnings":len(self.warnings),"criticals":len(self.criticals),"is_clean":self.is_clean,"issues":[{"check":i.check,"severity":i.severity,"description":i.description,"affected_rows":i.affected_rows,"details":i.details} for i in self.issues]}
...
class DataQualityChecker:
    ...
    def check(self, df, symbol):
        report = DataQualityReport(symbol=symbol, timeframe=self._timeframe, exchange=self._exchange, rows=len(df) if df is not None else 0, checked_at=datetime.now(timezone.utc).isoformat(), git_hash=_get_git_hash())
        if df is None or df.empty:
            report.issues.append(QualityIssue(check="empty_dataset",severity="critical",description="DataFrame vacio",affected_rows=0))
            return report
        ...
        if report.is_clean: logger.debug("Data quality OK | {}/{} exchange={} rows={}", symbol, self._timeframe, self._exchange, len(df))
        else:
            level = "error" if report.has_critical_issues else "warning"
            getattr(logger, level)("Data quality issues | {}/{} exchange={} warnings={} criticals={}", symbol, self._timeframe, self._exchange, len(report.warnings), len(report.criticals))
        return report
    ...
    def _check_future_timestamps(ts, report):
        ...
        n = int((ts > now).sum())
        if n > 0: report.issues.append(QualityIssue(check="future_timestamps",severity="critical",description=f"{n} timestamps en el futuro",affected_rows=n,details={"max_ts":str(ts.max()),"now":str(now)}))

    def _check_gaps(self, ts, report):
        if self._tf_ms is None: return
        ts_s = ts.sort_values().dropna()
        if len(ts_s) < 2: return
        ...
        report.issues.append(QualityIssue(check="temporal_gaps",severity=sev,description=f"{n} gaps ({missing} velas faltantes)",affected_rows=n,details={"gap_pct":round(pct*100,2),"missing_bars":missing,"largest_gap_s":round(float(diffs[mask].max()/1000),1)}))
    ...
    def _check_outliers_rolling_zscore(df, report):
        if "close" not in df.columns or len(df) < _ZSCORE_WINDOW + 1: return
        p = df["close"].dropna()
        if len(p) < _ZSCORE_WINDOW + 1: return
        ...
        if n > 0: report.issues.append(QualityIssue(check="price_outliers_zscore",severity="warning",description=f"{n} spikes via rolling z-score (window={_ZSCORE_WINDOW})",affected_rows=n,details={"method":"rolling_zscore","window":_ZSCORE_WINDOW,"threshold":_ZSCORE_THRESHOLD,"max_zscore":round(float(z.max()),2) if not z.isna().all() else None}))

    def _check_flatlines(self, df, report):
        if not all(c in df.columns for c in ("high","low","close")): return
