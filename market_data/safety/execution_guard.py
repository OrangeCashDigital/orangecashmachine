...
Kill switch + runtime guard para pipelines de ejecución.

Diseño
------
El guard se construye en entrypoint.run() y se pasa por parámetro
a cualquier componente que necesite verificar si debe continuar.
El contrato es explícito: si recibes un ExecutionGuard, eres cancelable.
...
Uso
---
    guard = ExecutionGuard(max_errors=10, max_runtime_s=3600)
    guard.start()

    # En el pipeline:
    guard.check()          # lanza si debe parar
    guard.record_error()   # cuenta errores consecutivos
    guard.record_success() # resetea contador
...
class ExecutionGuard:
    ...
    Parameters
    ----------
    max_errors    : int   — errores consecutivos antes de stop automático (0 = desactivado)
    max_runtime_s : float — segundos máximos de ejecución (0 = desactivado)
    """
    ...
    def trigger(self, reason: str) -> None:
        ...
        Thread-safe. Idempotente: si ya está abierto, preserva la razón original.
        """
        with self._lock:
            if not self._stop_event.is_set():
                self._stop_event.set()
    ...
    def record_error(self, reason: str = "") -> None:
        ...
        with self._lock:
            ...
            if self.max_errors > 0 and self._consecutive_errors >= self.max_errors:
                ...
                # Trigger sin lock (ya lo tenemos) — llamar _stop_event directamente
                if not self._stop_event.is_set():
                    self._stop_event.set()
