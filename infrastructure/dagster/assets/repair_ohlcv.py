...
def make_repair_ohlcv_asset(exchange_name: str, market_type: str = "spot"):
    ...
    def _repair_ohlcv_asset(
        ...
        if pairs_ok == 0:
            context.log.warning(
                f"repair skip — bronze tuvo 0 pares exitosos | exchange={exchange_name} market={market_type}"
            )
            yield Output(
        ...
        context.log.info(f"Repair iniciando | exchange={exchange_name} market={market_type} symbols={symbols}")
