...
def make_bronze_checks(exchange_name: str, market_type: str) -> List:
    ...
    def _schema_check(context, ocm: OCMResource) -> AssetCheckResult:
        ...
        return AssetCheckResult(
            passed=passed,
            description=("Schema OK" if passed else f"Columnas faltantes: {sorted(missing_cols)}"),
            **({"severity": AssetCheckSeverity.ERROR} if not passed else {}),
            metadata={
    ...
    def _freshness_check(context, ocm: OCMResource) -> AssetCheckResult:
        ...
        return AssetCheckResult(
            passed=passed,
            description=(
                f"Dato fresco ({age_hours:.1f}h)" if passed else f"Dato desactualizado: {age_hours:.1f}h sin actualizar"
            ),
    ...
    def _gap_check(context, ocm: OCMResource) -> AssetCheckResult:
        ...
        return AssetCheckResult(
            passed=passed,
            description=("Sin gaps HIGH" if passed else f"{len(high_gaps)} gap(s) HIGH detectados en {symbol}/{tf}"),
            **({"severity": AssetCheckSeverity.WARN} if not passed else {}),
            metadata={
                "total_gaps": len(gaps),
